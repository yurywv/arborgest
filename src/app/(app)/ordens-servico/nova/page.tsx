...
export default async function NewWorkOrder({ searchParams }: { searchParams: Promise<{ arvore?: string; clientId?: string }> }) {
  ...
  const [clients, properties, trees, users, teams, tree] = await Promise.all([
    clientOptions(), propertyOptions(), treeOptions(), userOptions(), teamOptions(), sp.arvore ? resolveTreeCode(sp.arvore) : null,
  ]);
  ...
  return (
    <div className="mx-auto max-w-3xl">
      ...
      <WorkOrderForm clients={clients} properties={properties} trees={trees} users={users} teams={teams} preset={preset} />
    </div>
  );
}
