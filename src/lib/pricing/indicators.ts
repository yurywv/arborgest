...
export async function pricingIndicators(from: Date, to: Date) {
  ...
  const [created, sent, accepted, rejected, items, byClient, overrides] = await Promise.all([
    db.pricingEstimate.findMany({ where: { date: range }, select: { id: true, negotiatedTotal: true, effectiveMargin: true, calculatedTotal: true, itemsTotal: true, discountAmount: true, _count: { select: { items: true } } } }),
  ...
  const avgMargin = withItems.length ? withItems.reduce((a, e) => a.plus(dec(String(e.effectiveMargin))), dec(0)).div(withItems.length) : null;
